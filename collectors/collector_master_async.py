...
async def _run_collector_async(
        collector_name: str,
    ...
    if progress_callback:
        ...
        try:
            ...
            logger.debug(
                f"[COLLECTOR_MASTER_ASYNC] Progress callback error: {e}")
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                f"[COLLECTOR_ASYNC] Failed to record performance: {e}")
...
async def collect_all_async(
    ...
    logger.info(
        f"[COLLECTION_ASYNC] Starting async collection of {total} collectors")

    # Faza 4: Ograniczenie równoległości przy dużych skanach
    # (asyncio.Semaphore)
    max_concurrent = config.get("collectors.max_concurrent", None)
    if max_concurrent and total > max_concurrent:
        logger.info(
            f"[COLLECTION_ASYNC] Limiting concurrency to {max_concurrent} collectors")
...
def _save_raw_data(results: Dict[str, Any], output_dir: str):
    ...
    try:
        ...
        logger.info(
            f"[COLLECTOR_MASTER_ASYNC] Raw data saved to: {filename} ({file_size_mb:.2f} MB)")
    except Exception as e:
        logger.error(f"[COLLECTOR_MASTER_ASYNC] Failed to save raw data: {e}")


def collect_all_async_wrapper(
        save_raw=True,
        output_dir="output/raw",
        progress_callback=None):
    """
