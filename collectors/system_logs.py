...
def parse_xml_logs(xml_data, filter_levels=None):
    ...
    filter_levels_lower = [f.lower()
                           for f in filter_levels] if filter_levels else []
    ...
    try:
        ...
        for obj in objects:
            ...
            timestamp = record.get("TimeCreated") or record.get(
                "Time") or record.get("TimeCreated.SystemTime") or ""
            message = record.get("Message") or record.get(
                "Message.Message") or ""

            # Pobierz level - sprawdź różne możliwe nazwy (sprawdź wszystkie
            # możliwe klucze)
            level = (record.get("LevelDisplayName")
                     or record.get("Level")
                     or record.get("LevelName")
                     # Czasami level jest jako ID (1=Critical, 2=Error,
                     # 3=Warning, 4=Information)
                     or record.get("LevelId")
                     or "Information")
            ...
            event_id = record.get("Id") or record.get(
                "EventID") or record.get("EventId") or "N/A"
            ...
            if filter_levels_lower and level_normalized.lower() not in filter_levels_lower:
                ...
                if filtered_count <= 5:
                    logger.debug(
                        f"[SYSTEM_LOGS] Filtered event {filtered_count}: level='{level_normalized}' not in {filter_levels_lower}")
                continue
            ...
            if is_shadowcopy:
                logger.debug(
                    f"[SYSTEM_LOGS] Detected ShadowCopy event: {event_id} - {message[:100]}")

            logs.append(log_entry)

        logger.debug(
            f"[SYSTEM_LOGS] Parsed {parsed_count} events, filtered {filtered_count}, returned {len(logs)}")

        # De-duplikuj eventy (ten sam provider, ten sam event id, pierwsze 200
        # znaków message)
        ...
        logger.debug(
            f"[SYSTEM_LOGS] XML data sample (first 500 chars): {xml_data[:500]}")
...
def normalize_level(level):
    ...
    if any(ord(c) > 127 and c not in 'ąćęłńóśźżĄĆĘŁŃÓŚŹŻ' for c in level_str):
        # Spróbuj naprawić kodowanie - jeśli zawiera zniekształcone znaki,
        # sprawdź wzorce
