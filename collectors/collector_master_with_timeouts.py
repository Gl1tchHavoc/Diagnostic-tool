...
async def _run_collector_with_timeout(
    ...
    try:
        ...
        log_performance(f"Collector {collector_name}", duration_ms / 1000, "TIMEOUT")
        logger.warning(f"Collector {collector_name} timed out after {timeout_seconds}s")
...
async def collect_all_with_timeouts(
    ...
    logger.info(f"[COLLECTION_TIMEOUT] Starting collection of {total} collectors (timeout: {timeout_seconds}s)")
        ...
        if loop.is_running():
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    asyncio.run,
                    collect_all_with_timeouts(save_raw, output_dir, progress_callback, timeout_seconds)
                )
                return future.result()
        else:
            return asyncio.run(collect_all_with_timeouts(save_raw, output_dir, progress_callback, timeout_seconds))
    except RuntimeError:
        return asyncio.run(collect_all_with_timeouts(save_raw, output_dir, progress_callback, timeout_seconds))
