...
def collect(sample_duration=2):
    ...
    if samples:
        ...
        # Disk I/O
        disk_reads = [s["disk_io"].get("read_bytes", 0) for s in samples if "disk_io" in s]
        disk_writes = [s["disk_io"].get("write_bytes", 0) for s in samples if "disk_io" in s]
        if disk_reads:
            perf_data["disk"] = {
        ...
        # Network I/O
        net_sent = [s["network_io"].get("bytes_sent", 0) for s in samples if "network_io" in s]
        net_recv = [s["network_io"].get("bytes_recv", 0) for s in samples if "network_io" in s]
        if net_sent:
            perf_data["network"] = {
