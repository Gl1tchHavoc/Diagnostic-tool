...
class SystemLogsCollector(BaseCollector):
    ...
    def collect(self):
        ...
        # System logs już zbiera wszystkie kategorie, więc progress jest
        # symulowany
        self.set_progress(100.0, "Complete")
        return data
...
class WERCollector(BaseCollector):
    ...
    def collect(self):
        ...
        try:
            self._logger.info(
                "[wer] DEBUG: WERCollector.collect() CALLED - START")
            sys.stdout.flush()
            ...
            try:
                self._logger.info(
                    "[wer] DEBUG: About to call wer.collect() - BEFORE CALL")
                sys.stdout.flush()
                data = wer.collect()
                self._logger.info(
                    f"[wer] DEBUG: wer.collect() RETURNED, type: {type(data)}")
                sys.stdout.flush()

                # Sprawdź czy data nie jest None
                if data is None:
                    self._logger.error(
                        "[wer] DEBUG: wer.collect() returned None!")
                    sys.stdout.flush()
                    data = {}

                self._logger.debug(
                    f"[wer] DEBUG: data is dict: {isinstance(data, dict)}")
                if isinstance(data, dict):
                    self._logger.debug(
                        f"[wer] DEBUG: data has {len(data)} keys")
                    # Nie loguj wszystkich kluczy - może być za dużo
                    if 'grouped_crashes' in data:
                        try:
                            gc_type = type(data['grouped_crashes'])
                            gc_is_list = isinstance(
                                data['grouped_crashes'], list)
                            gc_len = len(
                                data['grouped_crashes']) if gc_is_list else 'N/A'
                            self._logger.debug(
                                f"[wer] DEBUG: grouped_crashes type: {gc_type}, is_list: {gc_is_list}, len: {gc_len}")
                        except Exception as e:
                            self._logger.warning(
                                f"[wer] DEBUG: Error checking grouped_crashes: {e}")
                            sys.stdout.flush()

                self._logger.info(
                    "[wer] DEBUG: About to return data from WERCollector.collect()")
                sys.stdout.flush()
            except Exception as e:
                self._logger.exception(
                    f"[wer] DEBUG: Exception in wer.collect(): {e}")
                sys.stdout.flush()
                raise
            ...
            # Deepcopy może powodować zawieszenie na dużych obiektach z
            # zagnieżdżonymi strukturami
            self._logger.debug(
                "[wer] DEBUG: Skipping deep copy (data already sanitized in wer.collect())")
            sys.stdout.flush()
            ...
            self._logger.exception(
                f"[wer] DEBUG: FATAL EXCEPTION in WERCollector.collect(): {e}")
            sys.stdout.flush()
            raise
