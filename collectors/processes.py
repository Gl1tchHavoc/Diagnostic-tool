...
def collect():
    ...
    try:
        ...
        for proc in psutil.process_iter(['pid',
            ...
            try:
                ...
                try:
                    ...
                    proc_info['cmdline'] = ' '.join(
                        proc.cmdline()) if proc.cmdline() else "N/A"
                    proc_info['username'] = proc.username(
                    ) if proc.username() else "N/A"
                    ...
                    high_memory_procs.append(
                        {
                            ...
                            "cpu_percent": cpu_percent,
                            "memory_percent": memory_percent,
                            "memory_mb": proc_info.get(
                                'memory_info',
                                {}).get(
                                'rss',
                                0) / (
                                1024 * 1024) if proc_info.get('memory_info') else 0,
                            "exe": proc_info.get(
                                'exe',
                                'N/A')})
                ...
                if any(
                    suspicious in name or suspicious in exe for suspicious in [
                        'temp',
                        'tmp',
                        'random',
                        'unknown']):
        ...
        # Ogranicz do 100 najważniejszych
        processes_data["all_processes"] = all_procs[:100]
        processes_data["high_cpu"] = sorted(
            high_cpu_procs,
            key=lambda x: x.get(
                'cpu_percent',
                0),
            reverse=True)[
            :20]
        processes_data["high_memory"] = sorted(
            high_memory_procs,
            key=lambda x: x.get(
                'memory_percent',
                0),
