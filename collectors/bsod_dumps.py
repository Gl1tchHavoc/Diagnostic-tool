    ...
    for dump_path in minidump_paths:
        if dump_path.exists():
            if dump_path.is_dir():
                ...
                dump_files = sorted(dump_path.glob("*.dmp"), key=lambda p: p.stat().st_mtime, reverse=True)
                for dump_file in dump_files[:5]:  # Tylko 5 najnowszych
                    try:
                        ...
                        if parsed_info.get('success'):
                            ...
                            logger.info(f"[BSOD_DUMPS] Parsed minidump: {dump_file.name}, STOP: {parsed_info.get('stop_code')}, Driver: {parsed_info.get('offending_driver')}")
                        
                        bsod_data["minidumps"].append(dump_info)
                    except Exception as e:
                        logger.warning(f"[BSOD_DUMPS] Error processing minidump {dump_file}: {e}")
            else:
                ...
                try:
                    ...
                    logger.warning(f"[BSOD_DUMPS] Error processing MEMORY.DMP: {e}")
    ...
    try:
        ...
        try:
            ...
            if whea_events:
                ...
                logger.info(f"[BSOD_DUMPS] Found {len(whea_events)} WHEA hardware errors")
        except ET.ParseError:
            pass
    except Exception as e:
