...
class BaseCollector(ABC):
    ...
    def set_progress(self, progress: float, subtask_name: str = None):
        ...
        if subtask_name:
            ...
            subtask = next(
                (s for s in self.subtasks if s['name'] == subtask_name), None)
            if subtask:
    ...
    def run(self):
        ...
        try:
            ...
            self._logger.debug(
                f"[{self.name}] DEBUG: About to call self.collect()")

            try:
                self._logger.debug(
                    f"[{self.name}] DEBUG: Calling self.collect() NOW")
                if self.name == "wer":
                    self._logger.info(
                        f"[{self.name}] DEBUG: WER - About to call collect()")
                collected_data = self.collect()
                self._logger.debug(
                    f"[{self.name}] DEBUG: self.collect() returned, "
                    f"type: {type(collected_data)}")
                if self.name == "wer":
                    self._logger.info(
                        f"[{self.name}] DEBUG: WER - collect() "
                        f"returned successfully")
                    self._logger.debug(
                        f"[{self.name}] DEBUG: WER - About to assign "
                        f"data to self.data")

                # ZABEZPIECZENIE: Dla WER, upewnij się, że dane są
                # bezpieczne przed przypisaniem
                # NIE używaj json.dumps() jako testu - może zawiesić
                # się na dużych danych!
                # Dane są już sanityzowane w wer.collect()
                # (datetime -> string, ograniczone rozmiary)
                if self.name == "wer" and isinstance(collected_data, dict):
                    # Tylko podstawowa walidacja typu, bez pełnej
                    # serializacji
                    self._logger.debug(
                        f"[{self.name}] DEBUG: WER data type validated "
                        f"(skipping JSON test to avoid hang)")

                self.data = collected_data
                self._logger.debug(
                    f"[{self.name}] DEBUG: self.data assigned successfully")
                if self.name == "wer":
                    self._logger.info(
                        f"[{self.name}] DEBUG: WER - self.data "
                        f"assigned successfully")
            except Exception as collect_error:
                self._logger.exception(
                    f"[{self.name}] DEBUG: Exception in self.collect(): "
                    f"{collect_error}")
                raise

            # DEBUG: Sprawdź dane przed zapisaniem
            self._logger.debug(
                f"[{self.name}] DEBUG: self.data assigned, "
                f"type: {type(self.data)}")
            if self.name == "wer":
                self._logger.debug(
                    f"[{self.name}] DEBUG: self.data type: "
                    f"{type(self.data)}")
                self._logger.debug(
                    f"[{self.name}] DEBUG: self.data is dict: "
                    f"{isinstance(self.data, dict)}")
                if isinstance(self.data, dict):
                    self._logger.debug(
                        f"[{self.name}] DEBUG: self.data keys: "
            ...
            self._logger.debug(
                f"[{self.name}] DEBUG: About to set status to DONE")
            ...
            self._logger.debug(
                f"[{self.name}] DEBUG: About to create result dict")
            result = {
                'name': self.name,
                'status': self.status,
                'progress': self.progress,
            ...
            if self.name == "wer":
                self._logger.debug(
                    f"[{self.name}] DEBUG: Returning result, "
                    f"type: {type(result)}")
                self._logger.debug(
                    f"[{self.name}] DEBUG: result['data'] type: "
                    f"{type(result.get('data'))}")
                if (isinstance(result.get('data'), dict)
                        and 'grouped_crashes' in result.get('data', {})):
                    self._logger.debug(
                        f"[{self.name}] DEBUG: result['data']"
                        f"['grouped_crashes'] type: "
                        f"{type(result['data']['grouped_crashes'])}")

            self._logger.debug(f"[{self.name}] DEBUG: About to return result")
            return result
        except Exception as e:
            self._logger.debug(
                f"[{self.name}] DEBUG: Exception caught in run(): "
                f"{type(e).__name__}: {str(e)}")
            error_msg = f"{type(e).__name__}: {str(e)}"
            self._logger.debug(f"[{self.name}] DEBUG: About to set_error")
            self.set_error(error_msg)
            self._logger.debug(
                f"[{self.name}] DEBUG: Error set, about to log exception")
            self._logger.exception(f"[{self.name}] Collection failed")

            self._logger.debug(
                f"[{self.name}] DEBUG: About to create error result dict")
            error_result = {
                'name': self.name,
                'status': self.status,
                'progress': self.progress,
            ...
            self._logger.debug(
                f"[{self.name}] DEBUG: About to return error result")
            return error_result
