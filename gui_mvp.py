...
class DiagnosticsGUIMVP:
    ...
    def setup_widgets(self):
        ...
        self.collectors_tree.bind("<Double-1>", self.on_collector_double_click)  # Podwójne kliknięcie
        self.collectors_tree.bind("<Button-3>", self.on_collector_right_click)  # Prawy przycisk
        self.collectors_tree.bind("<Button-1>", self.on_collector_click)  # Pojedyncze kliknięcie
        
        # Scrollbar dla TreeView
        scrollbar_left = ttk.Scrollbar(left_frame, orient=tk.VERTICAL, command=self.collectors_tree.yview)
        ...
        self.collector_context_menu.add_command(label="🔍 Run Collector", command=self.run_selected_collector)
        self.collector_context_menu.add_command(label="👁️ View Data", command=self.view_collector_data)
        self.collector_context_menu.add_separator()
        self.collector_context_menu.add_command(label="🔄 Refresh Status", command=self.refresh_collector_status)
    ...
    def update_collectors_list(self):
        ...
        for name, info in sorted(collectors.items()):
            self.collectors_tree.insert("", tk.END, text=name, values=("Pending",))
    
    def update_collector_status(self, collector_name: str, status: str, error: str = None, data: dict = None):
        ...
        for item in self.collectors_tree.get_children():
            if self.collectors_tree.item(item, "text") == collector_name:
                ...
                status_text = status
                if error:
                    status_text = f"{status} ({error[:30]}...)" if len(error) > 30 else f"{status} ({error})"
    ...
    def run_full_scan(self):
        ...
        try:
            ...
            collected_data = collect_all(
                ...
                output_dir=self.config.get("output.raw_output_dir", "output/raw"),
            ...
            processed_data = analyze_all(collected_data, progress_callback=self.update_progress)
    ...
    def on_collector_click(self, event):
        ...
        item = self.collectors_tree.selection()[0] if self.collectors_tree.selection() else None
        if item:
    ...
    def on_collector_right_click(self, event):
        ...
        if item:
            ...
            self.selected_collector = collector_name
            try:
                self.collector_context_menu.tk_popup(event.x_root, event.y_root)
            finally:
                ...
                if self.selected_collector in collectors:
                    ...
                    if isinstance(collector_result, dict):
                        ...
                        if data:
                            ...
                            messagebox.showinfo("No Data", f"No data available for {self.selected_collector}")
                    else:
                        self.display_collector_data(self.selected_collector, collector_result)
                else:
                    messagebox.showinfo("No Data", f"No data available for {self.selected_collector}. Please run the collector first.")
            else:
    ...
    def run_single_collector(self, collector_name: str):
        ...
        if not collector_name:
            messagebox.showwarning("No Selection", "Please select a collector first")
            return
        ...
        if not collector_info:
            messagebox.showerror("Error", f"Collector '{collector_name}' not found in registry")
            return
        ...
        thread = Thread(target=self._run_single_collector_thread, args=(collector_name, collector_info), daemon=True)
        thread.start()
    
    def _run_single_collector_thread(self, collector_name: str, collector_info: dict):
        ...
        try:
            ...
            self.root.after(0, self._update_collector_after_run, collector_name, standardized_result)
    ...
    def _update_collector_after_run(self, collector_name: str, result: dict):
        ...
        if status == "Collected" and data:
            ...
            self.update_status(f"Collector '{collector_name}' completed successfully")
        else:
            self.update_status(f"Collector '{collector_name}' failed: {error}" if error else f"Collector '{collector_name}' completed")
    ...
    def refresh_collector_status(self):
        ...
        if self.selected_collector:
            ...
            if self.selected_collector in self.collector_data_cache:
                ...
                messagebox.showinfo("No Data", f"No cached data for {self.selected_collector}. Please run the collector first.")
