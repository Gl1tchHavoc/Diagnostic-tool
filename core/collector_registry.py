...
class CollectorRegistry:
    ...
    def register(self, name: str, collect_func: Callable, description: str = "",
        ...
        logger.debug(
            f"[COLLECTOR_REGISTRY] Registered collector: {name} ({category})")
    ...
    def get_all(self, enabled_only: bool = False) -> Dict[str, Dict]:
        ...
        """
        if enabled_only:
            return {name: info for name, info in self._collectors.items()
    ...
    def set_enabled(self, name: str, enabled: bool) -> bool:
        ...
        if name in self._collectors:
            ...
            logger.info(
                f"[COLLECTOR_REGISTRY] {'Enabled' if enabled else 'Disabled'} collector: {name}")
            return True
        return False
...
def register_all_collectors():
    ...
    logger.info(
        f"[COLLECTOR_REGISTRY] Registered {len(registry.get_all())} collectors")
