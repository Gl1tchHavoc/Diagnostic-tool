...
class PerformanceMonitor:
    ...
    def record_collector(self, collector_name: str, execution_time_ms: int,
        ...
        logger.debug(
            f"[PERFORMANCE] Collector {collector_name}: {execution_time_ms}ms, status={status}")
    ...
    def record_processor(self, processor_name: str, execution_time_ms: int,
        ...
        logger.debug(
            f"[PERFORMANCE] Processor {processor_name}: {execution_time_ms}ms, status={status}")
    ...
    def get_all_stats(self) -> Dict:
        ...
        session_duration = (
            datetime.now()
            - self.session_start).total_seconds()
    ...
    def log_summary(self):
        ...
        logger.info("=" * 60)
        logger.info(
            f"Session duration: {stats['session_duration_seconds']:.2f}s")
        logger.info(
            f"Total collector runs: {stats['summary']['total_collector_runs']}")
        logger.info(
            f"Total processor runs: {stats['summary']['total_processor_runs']}")
