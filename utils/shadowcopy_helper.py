...
def is_shadowcopy_path(path_or_message):
    ...
    for pattern in SHADOWCOPY_PATTERNS:
        if re.search(pattern, path_upper, re.IGNORECASE):
            logger.debug(f"[SHADOWCOPY] Detected ShadowCopy pattern in: {path_or_message[:100]}")
            return True
...
def filter_shadowcopy_errors(errors):
    ...
    for error in errors:
        ...
        if is_shadowcopy_path(text_to_check):
            ...
            logger.debug(f"[SHADOWCOPY] Classified as ShadowCopy error: {text_to_check[:100]}")
        else:
            real_errors.append(error)
    
    logger.info(f"[SHADOWCOPY] Filtered {len(errors)} errors: {len(real_errors)} real, {len(shadowcopy_errors)} ShadowCopy")
    ...
    try:
        ...
        if output:
            ...
            try:
                ...
                shadowcopy_info["vss_service_status"] = vss_info.get("Status", "Unknown")
                shadowcopy_info["vss_service_starttype"] = vss_info.get("StartType", "Unknown")
            except:
                pass
        ...
        shadowcopy_info["status"] = "ok" if shadowcopy_info.get("vss_service_status") == "Running" else "warning"
