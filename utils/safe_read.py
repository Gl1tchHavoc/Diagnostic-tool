...
def safe_read(file_path, encodings=None, fallback_encoding=None):
    ...
    for encoding in encodings:
        try:
            with open(file_path, 'r', encoding=encoding, errors='strict') as f:
                content = f.read()
                logger.debug(
                    f"[SAFE_READ] Successfully read {file_path} with encoding: {encoding}")
                return content, encoding, None
        except UnicodeDecodeError as e:
            logger.debug(
                f"[SAFE_READ] UnicodeDecodeError with {encoding}: {e}")
            continue
        except Exception as e:
            logger.debug(
                f"[SAFE_READ] Error reading {file_path} with encoding {encoding}: {type(e).__name__}: {e}")
            continue
    ...
    try:
        with open(file_path, 'r', encoding=fallback_encoding, errors='replace') as f:
            content = f.read()
            logger.warning(
                f"[SAFE_READ] Read {file_path} with fallback encoding {fallback_encoding} (some characters may be replaced)")
    ...
    try:
        ...
        with open(file_path, 'rb') as f:
            ...
            if encoding and confidence > 0.7:
                logger.debug(
                    f"[SAFE_READ] Detected encoding: {encoding} (confidence: {confidence:.2f})")
                return encoding
            else:
                logger.debug(
                    f"[SAFE_READ] Low confidence encoding detection: {encoding} (confidence: {confidence:.2f})")
                return None
    except ImportError:
        logger.debug(
            "[SAFE_READ] chardet not available, skipping auto-detection")
        return None
    except Exception as e:
        logger.debug(
            f"[SAFE_READ] Error detecting encoding: {type(e).__name__}: {e}")
        return None
