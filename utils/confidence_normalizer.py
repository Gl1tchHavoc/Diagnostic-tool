...
def normalize_confidence(confidence_score, min_score=0, max_score=100, target_min=0, target_max=100):
    """
    ...
    try:
        ...
        normalized = ((confidence_score - min_score) / (max_score - min_score)) * (target_max - target_min) + target_min
        
        # Ogranicz do zakresu docelowego
        normalized = max(target_min, min(target_max, normalized))
        
        logger.debug(f"[CONFIDENCE_NORMALIZER] Normalized {confidence_score} to {normalized}")
        return round(normalized, 2)
    
    except (TypeError, ValueError) as e:
        logger.warning(f"[CONFIDENCE_NORMALIZER] Error normalizing confidence {confidence_score}: {e}")
        return 0.0
...
def calculate_weighted_confidence(scores, weights=None):
    ...
    if len(weights) != len(scores):
        logger.warning(f"[CONFIDENCE_NORMALIZER] Weights length mismatch, using equal weights")
        weights = [1.0] * len(scores)
    ...
    try:
        ...
        logger.debug(f"[CONFIDENCE_NORMALIZER] Calculated weighted confidence: {weighted_avg} from {len(scores)} scores")
        return round(weighted_avg, 2)
    
    except Exception as e:
        logger.warning(f"[CONFIDENCE_NORMALIZER] Error calculating weighted confidence: {e}")
        return 0.0
...
def apply_confidence_decay(base_confidence, age_hours, decay_rate=0.1):
    ...
    try:
        ...
        logger.debug(f"[CONFIDENCE_NORMALIZER] Applied decay: {normalized} -> {decayed_confidence} (age: {age_hours}h, rate: {decay_rate})")
        return round(decayed_confidence, 2)
    
    except Exception as e:
        logger.warning(f"[CONFIDENCE_NORMALIZER] Error applying confidence decay: {e}")
