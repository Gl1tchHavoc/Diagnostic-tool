...
# Okno czasowe dla de-duplikacji (zdarzenia w tej samej sekundzie są
...
def deduplicate_events(events, time_window_seconds=DEDUP_TIME_WINDOW_SECONDS):
    ...
    for event in events:
        ...
        # Użyj pierwszych 100 znaków wiadomości
        key = f"{event_id}:{message[:100]}"
        ...
        if len(group) == 1:
            ...
            for time_group in grouped_by_time:
                if len(time_group) == 1:
                    ...
                    main_event['first_occurrence'] = min(
                        timestamps) if timestamps else main_event.get('timestamp', '')
                    main_event['last_occurrence'] = max(
                        timestamps) if timestamps else main_event.get('timestamp', '')

                    deduplicated.append(main_event)

                    logger.debug(
                        f"[DEDUP] Deduplicated {len(time_group)} events: {key[:50]}... (occurrences: {len(time_group)})")

    logger.info(
        f"[DEDUP] Deduplicated {len(events)} events to {len(deduplicated)} unique events")
