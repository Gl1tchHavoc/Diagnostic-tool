...
def is_false_disk_warning(message):
    ...
    for keyword in NON_DISK_KEYWORDS:
        if keyword in message_lower:
            logger.debug(f"[WARNING_CLASSIFIER] False DISK_WARNING detected (keyword: {keyword}): {message[:100]}")
            return True
...
def classify_warning(message, event_id=None):
    ...
    if is_false_disk_warning(message):
        ...
        network_keywords = ["url", "http", "upnp", "ssdpsrv", "wmpnetworksvc", "winhttp", "web services", "network", "tcp", "udp", "dns", "dhcp"]
    ...
    disk_keywords = ["disk", "volume", "ntfs", "file system", "i/o error", "read error", "write error", "bad block", "bad sector"]
