...
def correlate_events(processed_data, time_window_minutes=10):
    ...
    logger.info(f"[EVENT_CORRELATION] After deduplication: {len(all_events)} unique events")
    
    # Parsuj timestampy i sortuj
    for event in all_events:
        event['parsed_timestamp'] = parse_event_timestamp(event.get('timestamp', ''))
    ...
    logger.info(f"[EVENT_CORRELATION] Generated timeline with {len(timeline)} time windows, {len(correlations)} correlations, {len(root_causes)} root causes")
...
def create_timeline(events, time_window_minutes):
    ...
    for event in events:
        ...
        if current_window_start is None or (event_time - current_window_start).total_seconds() > time_window_minutes * 60:
            ...
            if current_window_events:
                timeline.append({
                    ...
                    'window_end': (current_window_start + timedelta(minutes=time_window_minutes)).isoformat() if current_window_start else None,
                    'events': current_window_events,
                    'event_count': len(current_window_events),
                    'max_priority': max([e.get('priority', 0) for e in current_window_events]) if current_window_events else 0
                })
...
def map_dependencies(events, time_window_minutes):
    ...
    for component, comp_events in component_events.items():
        ...
        for i in range(len(comp_events) - 1):
            ...
            time_diff = (event2.get('parsed_timestamp') - event1.get('parsed_timestamp')).total_seconds() / 60
...
def generate_root_cause_description(event, correlations):
    ...
    # Sprawdź korelacje
    related_correlations = [c for c in correlations if c.get('event1') == event_type or c.get('event2') == event_type]
...
def find_related_events(event, all_events, correlations):
    ...
    for corr in correlations:
        if corr.get('event1') == event_type or corr.get('event2') == event_type:
            # Znajdź powiązany event
            related_type = corr.get('event2') if corr.get('event1') == event_type else corr.get('event1')
            for e in all_events:
